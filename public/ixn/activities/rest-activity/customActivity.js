define([
    ...
    var steps = [ // initialize to the same value as what's set in config.json for consistency
        { "label": "Step 1", "key": "step1", "wizardVisible": true },
    ...
    function onRender() {
        ...
        $('#addoutargument').on('click',onAddOutArg);

        $('#toggleWizard').click(function () {
            steps[0].wizardVisible = !steps[0].wizardVisible; // toggle visibility
            connection.trigger('updateSteps', steps);
        });
    ...
    function initialize (data) {
        ...
        if (payload.errors) {
            $('#errors').text(JSON.stringify(payload.errors, null, 2));
        } else {
            $('#errors').text('None');
        }
    ...
    function onAddOutArg () {
        // Ensure payload.arguments.execute.outArguments
        payload['arguments'] = payload['arguments'] || {};
        payload['arguments'].execute = payload['arguments'].execute || {};
        payload['arguments'].execute.outArguments = payload['arguments'].execute.outArguments || [];

        // Ensure payload.schema.arguments.execute.outArguments
        payload.schema = payload.schema || {};
        payload.schema['arguments'] = payload.schema['arguments'] || {};
        payload.schema['arguments'].execute = payload.schema['arguments'].execute || {};
        payload.schema['arguments'].execute.outArguments = payload.schema['arguments'].execute.outArguments || [];

        // Add new randomly generated out argument
        var outArgName = ('myOutArg' + Math.random()).replace(/\./, '');

        var outArg = {};
        outArg[outArgName] = '';
        payload['arguments'].execute.outArguments.push(outArg);

        var outArgSchema = {};
        outArgSchema[outArgName] = {
            dataType: 'Text',
            isNullable: false,
            direction: 'Out',
            access: 'Visible'
        };
        payload.schema['arguments'].execute.outArguments.push(outArgSchema);
